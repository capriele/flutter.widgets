...
class ItemScrollController {
  ...
  void _attach(_ScrollablePositionedListState scrollableListState, _ListDisplayDetails primary, _ListDisplayDetails secondary) {
    //assert(_scrollableListState == null && primaryScrollController == null && secondaryScrollController == null);
...
class _ScrollablePositionedListState extends State<ScrollablePositionedList> with TickerProviderStateMixin {
  ...
  Future<void> _startScroll({
    ...
    final itemPosition = primary.itemPositionsNotifier.itemPositions.value.firstWhereOrNull((ItemPosition itemPosition) => itemPosition.index == index);
      ...
      startAnimationCallback = () {
        SchedulerBinding.instance!.addPostFrameCallback((_) {
          ...
          secondary.scrollController.jumpTo(-direction *
              (_screenScrollCount * primary.scrollController.position.viewportDimension - alignment * secondary.scrollController.position.viewportDimension));

          startCompleter
              .complete(primary.scrollController.animateTo(primary.scrollController.offset + direction * scrollAmount, duration: duration, curve: curve));
  ...
  void _updatePositions() {
    final itemPositions =
        primary.itemPositionsNotifier.itemPositions.value.where((ItemPosition position) => position.itemLeadingEdge < 1 && position.itemTrailingEdge > 0);
    if (itemPositions.isNotEmpty) {
      PageStorage.of(context)!.writeState(context, itemPositions.reduce((value, element) => value.itemLeadingEdge < element.itemLeadingEdge ? value : element));
    }
